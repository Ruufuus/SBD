...
CONSTRAINT fk_studio_name FOREIGN KEY (studio_name) REFERENCES studio(name) ON DELETE,UPDATE CASCADE,
...
CONSTRAINT medium_medium_constraint check (type in ('ANIME','MANGA','LIGHT NOVEL')),
CONSTRAINT medium_pk PRIMARY KEY (id),
CONSTRAINT medium_integrity_constraint check( (type = 'ANIME' AND studio_name IS NOT NULL) OR ((type IN ('MANGA', 'LIGHT NOVEL')
